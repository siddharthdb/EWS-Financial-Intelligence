...
Use Kafka Streams for canonical Kafka event consumption, deduplication, keyed state, rolling/bucketed feature calculations, moderate joins/aggregations, feature publication, deterministic signal-policy evaluation and signal-episode state.

Configure `processing.guarantee=exactly_once_v2` for material Kafka-to-Kafka stateful processing after operational validation. Use `num.standby.replicas=1` as the initial production resilience baseline where capacity permits.
...
- Kafka remains transport and state-recovery backbone;
...
- state restore/rebalance behaviour must be capacity-tested;
- topology complexity must be monitored to avoid forcing workloads into Streams when Flink is materially more appropriate.

## Production guardrails

- Stateful processing capacity includes active state, changelogs and standby state.
- Replay/backfill workloads cannot be allowed to starve live EWS processing.
- Exactly-once-v2 covers the Kafka Streams transaction boundary; it does not make external DB/HTTP/workflow side effects exactly once.
- Topic partitioning bounds useful parallelism.
- Broker/Streams failure and state restoration must be tested against defined freshness/RTO targets.
