...
| 2.8 | Expand priority signal contracts P10–P34 implementations | DONE (partial — P18 REQUIRED_MONITORING_INFORMATION_DELAY only; real XBRL balance-sheet facts (Assets/Liabilities/StockholdersEquity) are now extracted via `financial.statement.validated` (see 2.1), unblocking a future leverage/solvency-ratio feature and signal, but no such feature/signal is implemented yet — every other P10–P34 contract still needs additional XBRL concepts (income-statement, cash-flow) this platform doesn't parse yet) | 2026-09-24 — financial_statement_filing_delay_days + REQUIRED_MONITORING_INFORMATION_DELAY (P18) |
