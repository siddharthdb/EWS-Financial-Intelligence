...
| # | Item | Status | Build log entry |
|---|---|---|---|
| 2.1 | US SEC/EDGAR source adapter | DONE (partial — most-recent 10-K/10-Q detection plus real XBRL fact extraction for that exact filing: balance-sheet concepts (Assets/Liabilities/StockholdersEquity/AssetsCurrent/LiabilitiesCurrent) and now duration concepts (OperatingIncomeLoss/NetCashProvidedByUsedInOperatingActivities) with period-disambiguation logic to pick the correct discrete period among a single accession's overlapping/comparative entries; 8-K item classification, CIK watch-list, and entity resolution not yet implemented) | 2026-09-25 — operating_income/operating_cash_flow + OPERATING_PROFIT_MATERIAL_DECLINE (P13)/OPERATING_CASH_FLOW_NEGATIVE (P14) |
...
| 2.8 | Expand priority signal contracts P10–P34 implementations | DONE (partial — P18 REQUIRED_MONITORING_INFORMATION_DELAY, P12 LEVERAGE_DERIORATION, P11 CURRENT_RATIO_DERIORATION, and now P13 OPERATING_PROFIT_MATERIAL_DECLINE (`operating_income`, "against history" case only) and P14 OPERATING_CASH_FLOW_NEGATIVE (`operating_cash_flow`), both from the newly-extracted XBRL duration concepts. Remaining: P10 DSCR (needs a debt-service schedule this platform doesn't model), P13's "against plan/peers" cases, P15-P17 (receivable/inventory days — needs Revenues/CostOfGoodsSold concepts not yet extracted), P19-P34 (legal/rating/ownership/market events — need entirely different external sources, not XBRL)) | 2026-09-25 — operating_income/operating_cash_flow + OPERATING_PROFIT_MATERIAL_DECLINE (P13)/OPERATING_CASH_FLOW_NEGATIVE (P14) |
