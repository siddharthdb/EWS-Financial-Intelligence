...
| 0.9 | Shared schema artifacts/references for repeated envelope types | DONE | 2026-09-24 — shared semantic-scope schema artifact; 2026-09-25 — data-quality-state + T1-T4 tier shared schema artifacts. `semanticScope`, the quality `state` enum, and the T1-T4 tier enum (both non-nullable and nullable variants, to avoid weakening either strict or optional use sites) are now all shared. No further genuinely-identical repeated envelope fields remain -- entity-type enums that looked similar across files were checked and confirmed to be deliberately different subsets per context, not true duplicates (see the 2026-09-24 entry). |
...
| 1.14 | `wc_utilization_ratio` feature + `UTILIZATION_HIGH` (P05) signal | DONE (`wc_utilization_ratio`/`UTILIZATION_HIGH` (P05, 2026-09-24), `wc_utilization_delta_30d`/`UTILIZATION_SPIKE` (P06, see 2.3), and now `wc_available_headroom`/`LIMIT_EXCESS_RECURRING` (P07) all implemented — see 2026-09-25 entry. `limit_excess_days_30d` (the catalogue's more precise windowed companion feature) remains unimplemented; P07 instead uses a coarser two-consecutive-negative-observations proxy, documented in `LimitExcessRecurringSignalPolicyLoader`) | 2026-09-25 — wc_available_headroom + LIMIT_EXCESS_RECURRING (P07) |
...
| # | Item | Status | Build log entry |
|---|---|---|---|
| 2.1 | US SEC/EDGAR source adapter | DONE (partial — most-recent 10-K/10-Q detection plus real XBRL fact extraction for that exact filing: balance-sheet concepts (Assets/Liabilities/StockholdersEquity/AssetsCurrent/LiabilitiesCurrent/AccountsReceivableNetCurrent/InventoryNet), duration concepts with period-disambiguation (OperatingIncomeLoss/NetCashProvidedByUsedInOperatingActivities/RevenueFromContractWithCustomerExcludingAssessedTax with a fallback to the older Revenues concept/CostOfGoodsAndServicesSold), and the discrete period's length (`periodDays`) alongside the facts, needed to normalize day-count ratios; 8-K item classification, CIK watch-list, and entity resolution not yet implemented) | 2026-09-25 — inventory_days + INVENTORY_DAYS_DERIORATION (P16) |
| 2.2 | India regulatory source adapter(s) | NOT_STARTED | blocked — no keyless public API found; see human-decision list |
| 2.3 | Statistical/anomaly detection engine (method `S`/`A` signals) | DONE (partial — method-S: `wc_utilization_delta_30d` + `UTILIZATION_SPIKE` (P06); financial-statement (method R): `net_income` + `NET_LOSS_EMERGENCE`; true anomaly-method (A) detection and the remaining 04-signal-taxonomy.md §4 signals (`EBITDA_MARGIN_DERIORATION`, `NET_WORTH_EROSION`, `CASH_FLOW_PROFIT_DIVERGENCE`, `CASH_CONVERSION_CYCLE_DIVERGENCE`, and the NLP-method ones) remain unimplemented) | 2026-09-24 — wc_utilization_delta_30d + UTILIZATION_SPIKE (P06): first statistical (method S) signal; 2026-09-25 — net_income + NET_LOSS_EMERGENCE (roadmap 2.3) |
...
| 2.8 | Expand priority signal contracts P10–P34 implementations | DONE (partial — P18 REQUIRED_MONITORING_INFORMATION_DELAY, P12 LEVERAGE_DERIORATION, P11 CURRENT_RATIO_DERIORATION, P13 OPERATING_PROFIT_MATERIAL_DECLINE ("against history" case only), P14 OPERATING_CASH_FLOW_NEGATIVE, P15 RECEIVABLE_DAYS_DERIORATION, and now P16 INVENTORY_DAYS_DERIORATION (`inventory_days` = InventoryNet / CostOfGoodsAndServicesSold * periodDays). All six of P11-P16's XBRL-derivable balance-sheet/income-statement contracts are now implemented. Remaining: P10 DSCR (needs a debt-service schedule this platform doesn't model), P13's "against plan/peers" cases, P17 COVENANT_BREACH (needs covenant terms/measurement data this platform doesn't ingest at all), P19-P34 (legal/rating/ownership/market events — need entirely different external sources, not XBRL)) | 2026-09-25 — inventory_days + INVENTORY_DAYS_DERIORATION (P16) |
