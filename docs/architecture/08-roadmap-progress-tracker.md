...
| 2.8 | Expand priority signal contracts P10–P34 implementations | DONE (partial — P18 REQUIRED_MONITORING_INFORMATION_DELAY, P12 LEVERAGE_DERIORATION, P11 CURRENT_RATIO_DERIORATION, P13 OPERATING_PROFIT_MATERIAL_DECLINE ("against history" case only), P14 OPERATING_CASH_FLOW_NEGATIVE, and now P15 RECEIVABLE_DAYS_DERIORATION (`receivable_days` = AR / revenue * periodDays, with revenue-concept fallback for the pre-2018 XBRL taxonomy). Remaining: P10 DSCR (needs a debt-service schedule this platform doesn't model), P13's "against plan/peers" cases, P16 INVENTORY_DAYS_DERIORATION / P17 COVENANT_BREACH (P16 reuses the same periodDays machinery with InventoryNet/CostOfGoodsAndServicesSold — a reasonable next candidate; P17 needs covenant data this platform doesn't ingest), P19-P34 (legal/rating/ownership/market events — need entirely different external sources, not XBRL)) | 2026-09-25 — receivable_days + RECEIVABLE_DAYS_DERIORATION (P15) |
