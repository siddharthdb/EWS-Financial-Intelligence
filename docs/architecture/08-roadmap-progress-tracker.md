...
| # | Item | Status | Build log entry |
|---|---|---|---|
| 2.1 | US SEC/EDGAR source adapter | DONE (partial — most-recent 10-K/10-Q detection plus real balance-sheet XBRL fact extraction (Assets/Liabilities/StockholdersEquity/AssetsCurrent/LiabilitiesCurrent) for that exact filing; 8-K item classification, CIK watch-list, and entity resolution not yet implemented) | 2026-09-25 — current_ratio + CURRENT_RATIO_DERIORATION (P11) |
...
| 2.8 | Expand priority signal contracts P10–P34 implementations | DONE (partial — P18 REQUIRED_MONITORING_INFORMATION_DELAY, P12 LEVERAGE_DERIORATION, and now P11 CURRENT_RATIO_DERIORATION (`current_ratio` + a materially-worsening relative-decrease policy, reusing the same AssetsCurrent/LiabilitiesCurrent XBRL concepts now extracted alongside the balance-sheet ones); every other P10–P34 contract still needs additional XBRL concepts (income-statement, cash-flow, EBITDA) this platform doesn't parse yet) | 2026-09-25 — current_ratio + CURRENT_RATIO_DERIORATION (P11) |
