...
public final class PolicyValidatorCli {
  public static void main(String[] args) throws Exception {
    ...
    try(var paths=Files.list(dir)){
      for(Path p:paths.filter(x->x.toString().endsWith(".json")).sorted().toList()){
        ...
        for(var x:result.findings()) System.out.printf("  %s %s %s %s%n",x.severity(),x.ruleId(),x.code(),x.message());
  ...
  private static ObjectNode toEvidence(ObjectMapper m,JsonNode p,PolicyPublicationValidator.Result r){
    ...
    ObjectNode policy=o.putObject("policy"); policy.put("policyId",p.path("policyId").asText()); policy.put("version",p.path("version").asInt());
    o.put("policyArtifactHash",r.policyArtifactHash());
    ObjectNode sv=o.putObject("schemaValidation"); sv.put("valid",r.schemaValid()); sv.put("schemaRef","schemas/policies/risk-policy-v1.schema.json");
    ArrayNode se=sv.putArray("errors"); ArrayNode sem=null;
    ObjectNode sm=o.putObject("semanticValidation"); sm.put("valid",r.semanticValid()); ObjectNode snap=sm.putObject("registrySnapshot"); r.registrySnapshot().forEach(snap::put); sem=sm.putArray("findings");
    for(var f:r.findings()){ObjectNode n=m.createObjectNode();n.put("ruleId",f.ruleId());n.put("severity",f.severity().name());n.put("code",f.code());n.put("message",f.message());if(f.jsonPointer()==null)n.putNull("jsonPointer");else n.put("jsonPointer",f.jsonPointer());n.putNull("reference");if("SCHEMA".equals(f.ruleId()))se.add(n);else sem.add(n);}
    o.put("publishable",r.publishable()); o.put("validatedAt",Instant.now().toString()); o.put("validatorVersion","0.1.0"); o.putNull("traceId"); return o;
  }
