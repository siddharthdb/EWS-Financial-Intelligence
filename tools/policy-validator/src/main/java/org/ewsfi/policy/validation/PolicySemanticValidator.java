...
public final class PolicySemanticValidator {
    ...
    public List<Finding> validate(JsonNode policy, GovernedRegistry registry) {
        ...
        Set<String> policyEntityTypes = new HashSet<>(); for (JsonNode e: policy.path("scope").path("entityTypes")) policyEntityTypes.add(e.asText());
        ...
        for (JsonNode rule: policy.path("rules")) {
            ...
            if ("PRIORITY".equals(mode) && rule.path("enabled").asBoolean()) { int p=rule.path("priority").asInt(); if(!priorities.add(p)) findings.add(error("PSV-007","DUPLICATE_PRIORITY","Enabled PRIORITY rules must have unique priorities: "+p,ptr+"/priority")); }
        ...
        if(from.isTextual()&&to.isTextual()&&!to.asText().isBlank()) try { if(!java.time.Instant.parse(to.asText()).isAfter(java.time.Instant.parse(from.asText()))) findings.add(error("PSV-009","INVALID_EFFECTIVE_PERIOD","effectiveTo must be later than effectiveFrom","/effectivePeriod")); } catch(Exception ex){ findings.add(error("PSV-009","INVALID_EFFECTIVE_PERIOD","Effective timestamps must be ISO-8601 instants","/effectivePeriod")); }

        if("ACTIVE".equals(policy.path("lifecycle").asText())&&policy.path("governance").path("makerCheckerRequired").asBoolean()) {
            if(policy.path("governance").path("approvedBy").isNull()||policy.path("governance").path("approvedAt").isNull()) findings.add(error("PSV-011","MISSING_APPROVAL","ACTIVE maker-checker policy requires approval identity and timestamp","/governance"));
            if(policy.path("governance").path("simulationRunId").isNull()) findings.add(error("PSV-011","MISSING_SIMULATION","ACTIVE policy requires accepted simulation evidence","/governance/simulationRunId"));
        }
        String engine=policy.path("evaluation").path("engine").asText();
        if(("DMN".equals(engine)||"DROOLS".equals(engine))&&policy.path("evaluation").path("externalArtifactRef").isNull()) findings.add(error("PSV-012","MISSING_ENGINE_ARTIFACT",engine+" policy requires an approved artifact reference","/evaluation/externalArtifactRef"));
    ...
    private void validateExpression(JsonNode node, Map<String,JsonNode> parameters, GovernedRegistry registry, Set<String> entityTypes, List<Finding> findings, String ptr) {
        ...
        validateOperand(node.path("left"),parameters,registry,entityTypes,findings,ptr+"/left"); validateOperand(node.path("right"),parameters,registry,entityTypes,findings,ptr+"/right");
    }
