...
class RepositoryPolicyPublicationTest {
  ...
  private PolicyPublicationValidator validator;
  private GovernedRegistry registry;
  ...
  @BeforeEach void setup() throws Exception {
    ...
    registry=GovernedRegistry.from(
      read("registries/phase1-feature-registry-v1.json"),
      read("registries/phase1-signal-registry-v1.json"),
      read("registries/risk-dimensions-v1.json"));
  }
  ...
  @Test void invalidThresholdOrderingFixtureFailsPsv006() throws Exception {
    ...
    fixture.path("parameterOverrides").fields().forEachRemaining(e->{
      for(JsonNode p:policy.withArray("parameters"))
        if(p.path("name").asText().equals(e.getKey()))((ObjectNode)p).set("value",e.getValue());
    });
    var r=validator.validate(policy,constraints,registry,List.of());
    assertTrue(r.schemaValid());
    assertFalse(r.publishable());
    assertTrue(r.findings().stream().anyMatch(finding->
      finding.ruleId().equals("PSV-006")&&finding.code().equals("PARAMETER_RELATION_VIOLATION")));
  }
  ...
  @Test void overlappingVersionFixtureFailsPsv010() throws Exception {
    ...
    policy.with("effectivePeriod").put("effectiveFrom",candidate.path("effectiveFrom").asText());
    ArrayNode products=policy.with("scope").putArray("productTypes");
    candidate.path("productTypes").forEach(products::add);

    JsonNode old=fixture.path("existing");
    var existing=new PolicyGovernanceValidator.PolicyVersion(
      old.path("policyId").asText(),candidate.path("policyKey").asText(),old.path("version").asInt(),
      old.path("institutionId").asText(),set(old.path("entityTypes")),Set.of(),set(old.path("productTypes")),
    ...
    assertFalse(r.publishable());
    assertTrue(r.findings().stream().anyMatch(finding->
      finding.ruleId().equals("PSV-010")&&finding.code().equals("EFFECTIVE_VERSION_OVERLAP")));
  }

  private JsonNode read(String p)throws Exception {
    return m.readTree(Files.readString(root.resolve(p)));
  }

  private static Path findRoot() {
    Path p=Path.of(System.getProperty("user.dir")).toAbsolutePath();
    for(int i=0;i<5&&p!=null;i++,p=p.getParent())
      if(Files.exists(p.resolve("schemas/policies")))return p;
    throw new IllegalStateException("Repository root not found from user.dir");
  }

  private Set<String> set(JsonNode n) {
    Set<String>s=new HashSet<>();
    n.forEach(x->s.add(x.asText()));
    return s;
  }
}