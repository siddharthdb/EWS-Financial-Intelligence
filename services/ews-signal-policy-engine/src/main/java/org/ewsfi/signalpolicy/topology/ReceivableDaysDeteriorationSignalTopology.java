...
import org.ewsfi.signalpolicy.policy.ReceivableDaysDeteriorationSignalPolicyLoader;
import org.springframework.stereotype.Component;

/**
 * Builds the Kafka Streams topology evaluating the P15 RECEIVABLE_DAYS_DERIORATION policy
 * (docs/architecture/02a-priority-signal-contracts.md, {@link ReceivableDaysDeteriorationSignalPolicyLoader})
 * against {@code receivable_days} values on {@code ews.derived.feature}, producing
 ...
 */
@Component
public class ReceivableDaysDeteriorationSignalTopology {

    static final String FEATURE_TOPIC = "ews.derived.feature";
    static final String SIGNAL_TOPIC = "ews.derived.signal";
    private static final String TARGET_FEATURE_NAME = "receivable_days";
    private static final String STORE_NAME = "receivable-days-deterioration-transition-store";
    ...
    private String toSignalIfDeteriorated(String stateJson) {
        ReceivableDaysState state = parseState(stateJson);
        if (state.previousDays == null || state.currentDays == null) {
            return null;
        }
        boolean deteriorated =
                ReceivableDaysDeteriorationSignalPolicyLoader.evaluate(state.previousDays, state.currentDays);
        if (!deteriorated) {
            return null;
        }
    ...
    private String toStateJson(ReceivableDaysState state) {
        try {
            return objectMapper.writeValueAsString(state);
        } catch (Exception e) {
            throw new IllegalStateException("Failed to serialize ReceivableDaysState", e);
        }
    }

    private ReceivableDaysState parseState(String json) {
        try {
            return objectMapper.readValue(json, ReceivableDaysState.class);
        } catch (Exception e) {
            return new ReceivableDaysState(null, null, null, null, null, null);
        }
    }

    private String toSignalDetectedJson(ReceivableDaysState state) {
        ...
        JsonSignalDetected signal =
                new JsonSignalDetected(
                        signalId,
                        ReceivableDaysDeteriorationSignalPolicyLoader.SIGNAL_TYPE,
                        ReceivableDaysDeteriorationSignalPolicyLoader.SEMANTIC_SCOPE,
                        state.entityType,
                        state.entityId,
                        "PROPOSED",
                        "MEDIUM",
                        ...
                        now.toString(),
                        state.knowledgeTime,
                        ReceivableDaysDeteriorationSignalPolicyLoader.POLICY_ID,
                        ReceivableDaysDeteriorationSignalPolicyLoader.POLICY_VERSION,
                        "COMPLETE",
    ...
    /** Per-counterparty receivable_days transition state, tracked across observations. */
    public static class ReceivableDaysState {
