...
/**
 * Tests {@link NetIncomeFeatureTopology}'s stateless per-event extraction in isolation using
 * {@link TopologyTestDriver} -- no broker required.
 */
class NetIncomeFeatureTopologyTest {
    ...
    void setUp() {
        ...
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "net-income-feature-topology-test");
        props.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "dummy:1234");

        StreamsBuilder builder = new StreamsBuilder();
        new NetIncomeFeatureTopology().build(builder);
        Topology topology = builder.build();

        driver = new TopologyTestDriver(topology, props);
        inputTopic =
                driver.createInputTopic(
                        NetIncomeFeatureTopology.CANONICAL_TOPIC,
                        Serdes.String().serializer(),
                        Serdes.String().serializer());
        outputTopic =
                driver.createOutputTopic(
                        NetIncomeFeatureTopology.FEATURE_TOPIC,
    ...
    @Test
    void extractsNetIncomeLossFromTheValidatedFacts() {
        inputTopic.pipeInput("0000320193", validatedJson("0000320193", 29_789_000_000L), Instant.now());

        var outputs = outputTopic.readKeyValuesToList();

        assertThat(outputs).hasSize(1);
        JsonFeatureValue featureValue = parse(outputs.get(0).value);
        assertThat(featureValue.getFeatureName()).isEqualTo("net_income");
        assertThat(featureValue.getEntityType()).isEqualTo("COUNTERPARTY");
        assertThat(featureValue.getEntityId()).isEqualTo("0000320193");
        assertThat(Double.parseDouble(featureValue.getValueNumeric())).isEqualTo(29_789_000_000.0);
    }

    @Test
    void reportsANetLoss() {
        inputTopic.pipeInput("0000999999", validatedJson("0000999999", -2_000_000L), Instant.now());

        var outputs = outputTopic.readKeyValuesToList();
        assertThat(outputs).hasSize(1);
        assertThat(Double.parseDouble(parse(outputs.get(0).value).getValueNumeric())).isEqualTo(-2_000_000.0);
    }
    ...
    void missingFactsProduceNoOutput() {
        ...
        data.put("facts", Map.of("Assets", 383_266_000_000L)); // no NetIncomeLoss
    ...
    private static String validatedJson(String cik, long netIncome) {
        Map<String, Object> facts = new LinkedHashMap<>();
        facts.put("NetIncomeLoss", netIncome);
